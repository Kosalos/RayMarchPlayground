...
float scene(float3 pos,Control control) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        float pwr = control.power;
        float t = 8.0-sin(control.time / 3.0)*3.0;
        float wo = acos(z.z/r) * t + control.time;
        float wi = atan2(z.y,z.x) * pwr;
        
        dr =  pow(r, pwr-1.0)*pwr*dr + 1.0;
        
        z = pos + pow(r, pwr-1.0) * float3(sin(wo)*cos(wi), sin(wi)*sin(wo), cos(wo));
    }
